...
public class FreeMarkerService {
    ...
    public void createEntity(String tableName, String saveUrl, String packageName) throws Exception {
        ...
        if (applicationContext.containsBean(CommonsUtils.topCharSmall(entityName + "Mapper"))) {
            log.debug("数据表-[{}]基础代码已存在", tableName);
            return;
        }
        ...
        //生成serviceImpl
        freeMarkerTools.generate(table,"serviceImpl.ftl",saveUrl + "service/impl",entityName+"ServiceImpl.java");
        // 生成controller
        freeMarkerTools.generate(table,"controller.ftl",saveUrl + "controller",entityName+"Controller.java");
